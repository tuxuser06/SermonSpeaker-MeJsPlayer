...
/**
 * Plug-in to show the MeJsPlayer from http://mediaelementjs.com / https://github.com/johndyer/mediaelement / https://github.com/tuxuser06/mep-feature-simpleplaylist
 *
 * @since  5.4.0
 */
class PlgSermonspeakerMeJsPlayer extends SermonspeakerPluginPlayer
{
	/**
	...
	public function onGetPlayer($context, &$player, $items, $config)
	{
		...
		// toggle not used/implemented
//		$toggle         = $this->params->get('filetoggle');
		$toggle = false;
		$playlistbuttonorder = $this->params->get('playlistbuttonorder');
		...
		$this->player->mspace = '<div id="mediaspace' . $count . '"style="width:100%; height:100%; max-width: 100%">Loading Player...<source src="' . $dummyName . '"></div>';
		...
		if (is_array($items))
		{
			...
			if ($playlistbuttonorder == 1)
			{
			      $featuresStr = "['playlistfeature', 'prevtrack', 'playpause', 'nexttrack', 'current', 'progress', 'duration', 'volume', 'fullscreen']";
			}
			else
			{
			      $featuresStr = "['playlistfeature', 'playpause', 'prevtrack', 'nexttrack', 'current', 'progress', 'duration', 'volume', 'fullscreen']";
			}
			$eventListenerStr = $this->options['eventListener'];
		}
		else
		{
			$this->createSinglePlaylist($items);
			$featuresStr = "['playlistfeature', 'playpause', 'current', 'progress', 'duration', 'volume', 'fullscreen']";
			$eventListenerStr = "{}";
		}
...
//		$dimensions['videoheight'] = $this->params->get('vheight', '300px');
		...
		$this->player->script = '<script type="text/javascript">'
			. "var mePlayer = new MediaElementPlayer('#mediaspace" . $count . "', {"
			. "features: " . $featuresStr . ","
			. "alwaysShowControls: true,"
			. "isVideo: " . $isVideoStr . ","
			. "eventListener: " . $eventListenerStr . ","
			. "playlistElements:[" . $this->player->playlist['default'] . "],"
		...
		if (!self::$script_loaded)
		{
			...
			JHtml::_('script', 'media/plg_sermonspeaker_mejsplayer/mediaelement-and-player.min.js');
			JHtml::_('script', 'media/plg_sermonspeaker_mejsplayer/mep-feature-simpleplaylist.js');

			$doc->addScriptDeclaration("function ss_play(id){mePlayer.playPlaylistPosition(id);}");
...
//				');
	...
	private function setOptions()
	{
		...
		$file = 'plg_sermonspeaker_mejsplayer/mediaelementplayer.min.css';
		JHtml::_('stylesheet', $file, false, true, false);
		$file = 'plg_sermonspeaker_mejsplayer/mep-feature-simpleplaylist.css';
		JHtml::_('stylesheet', $file, false, true, false);
		$file = 'plg_sermonspeaker_mejsplayer/mep-highlight.css';
		JHtml::_('stylesheet', $file, false, true, false);
	...
	private function createMultiPlaylist($items)
	{
		...
		$this->options['eventListener'] = '{'
			. "'mep-position': function(event){"
			. 'for (var i = 0; jQuery("#sermon"+i).length; i++){'
			. 'jQuery("#sermon"+i).removeClass("ss-current");'
			. '}'
			. 'jQuery("#sermon"+mePlayer.playlistPosition).addClass("ss-current");'
			. 'var entry = mePlayer.playlistElements[mePlayer.playlistPosition];'
			. 'if (entry.duration > 0){'
			. 'time = new Array();'
